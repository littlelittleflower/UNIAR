...
def get_g_bidir(triples, args):#构建DGL双向图
    g = dgl.graph((torch.cat([triples[:, 0].T, triples[:, 2].T]),
                   torch.cat([triples[:, 2].T, triples[:, 0].T])))
    # max_idx_h = torch.max(triples[:, 0])  # 头实体最大索引
    # max_idx_t = torch.max(triples[:, 2])  # 尾实体最大索引
    # max_entity_idx = max(max_idx_h, max_idx_t)  # 总体最大索引
    #
    # print(f"Max entity index in triples: {max_entity_idx}")  # 检查索引范围

    g.edata['type'] = torch.cat([triples[:, 1].T, triples[:, 1].T + args.num_rel])
    return g

def get_g_bidir_num(triples, args):#构建DGL双向图
    g = dgl.graph((torch.cat([triples[:, 0].T, triples[:, 2].T]),
                   torch.cat([triples[:, 2].T, triples[:, 0].T])))
    max_idx_h = torch.max(triples[:, 0])  # 头实体最大索引
    max_idx_t = torch.max(triples[:, 2])  # 尾实体最大索引
...
def get_indtest_test_dataset_and_train_g(args):
    ...
    test_dataset = KGEEvalDataset(args, data['test'], num_ent, hr2t, rt2h)#KGEEvalDataset

    g = get_g_bidir(torch.LongTensor(data['train']), args)#用train构建双向DGL图嵌入，test构建测试集，valid没用
...
def get_num_rel_entity(args):
    data = pickle.load(open(args.data_path, 'rb'))
    num_rel = len(np.unique(np.array(data['train_graph']['train'])[:, 1]))

    # Extract head and tail entities and count unique entities
    train_data = np.array(data['train_graph']['train'])

    num_entity = len(np.unique(np.concatenate((train_data[:, 0], train_data[:, 2]))))

    return num_entity,num_rel
