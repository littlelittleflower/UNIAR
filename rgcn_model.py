...
class RGCNLayer(nn.Module):
    ...
    def msg_func(self, edges):
        ...
        src_feat = edges.src[self.input_]  # [E, in_dim]
        rel_type = edges.data['type'].long()  # [E]

        w = self.rel_weight.index_select(0, rel_type)  # [E, in_dim, out_dim]


        msg = torch.bmm(src_feat.unsqueeze(1), w).squeeze(1)  # [E, out_dim]
        curr_emb = torch.mm(edges.dst[self.input_], self.self_loop_weight)
        a = 1 / edges.dst['in_d'].float().reshape(-1, 1).to(msg.device)
...
class RGCN(nn.Module):
    ...
    def forward(self, g):
        if 'h' not in g.ndata:
            raise ValueError("Missing node input features: g.ndata['h'] not set!")
